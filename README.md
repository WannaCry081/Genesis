...
- **Repository permissions** — Release Please needs to open pull requests on your behalf. Go to **Settings → Actions → General → Workflow permissions**, enable **Read and write permissions**, and check **Allow GitHub Actions to create and approve pull requests**. Without this, the workflow will fail with a permissions error.
